...
let rec new_state (state : game_state) (command : string) : unit =
  ...
  if state.round = Start then (
    ...
    | cmnd ->
        print_endline
          "You have entered an illegal bet amount. Please try again";
        new_state { state with p1_bet = state.p1_bet } "PlaceHolder"
    (*AFTER P2 BETS, CARDS WILL BE DEALT OUT*))
  else if state.round = P2_bet then (
    print_endline
      ("Player 2 bet $" ^ string_of_int (bet_from_parse (parse command)));
